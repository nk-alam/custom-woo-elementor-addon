...
// Plugin constants
define('CUSTOM_WOO_ELEMENTOR_VERSION', '1.3.0');
...
final class Custom_Woo_Elementor_Plugin {
    ...
    public function save_custom_product_fields($post_id) {
        ...
        $custom_rating = $_POST['_custom_rating'] ?? '';
        update_post_meta($post_id, '_custom_rating', esc_attr($custom_rating));

        $custom_review_count = $_POST['_custom_review_count'] ?? '';
        update_post_meta($post_id, '_custom_review_count', esc_attr($custom_review_count));

        $default_variation = $_POST['_default_variation'] ?? '';
        update_post_meta($post_id, '_default_variation', esc_attr($default_variation));
    }
}
